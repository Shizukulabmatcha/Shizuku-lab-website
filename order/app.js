...
/* ---------- init ---------- */
async function loadDesignTheme() {
  document.documentElement.dataset.storeTheme = "zen";
  document.documentElement.dataset.retroPalette = "olive";
  try {
    const { data: rows, error } = await db.from("website_live").select("id,data").in("id", ["main_v2", "main"]);
    if (error) throw error;
    const row = (rows || []).find((item) => item.id === "main_v2") || (rows || []).find((item) => item.id === "main");
    document.documentElement.dataset.storeTheme = row?.data?.themes?.store || "zen";
    document.documentElement.dataset.retroPalette = row?.data?.themes?.retroPalette || "olive";
  } catch (error) { console.warn("Website theme unavailable", error); }
}
async function init() {
  state.loading = true;
  ...
  try {
    await Promise.all([ensureCustomerSession(), loadDesignTheme()]);
