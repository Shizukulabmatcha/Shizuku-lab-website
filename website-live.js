(function(){
  ...
  const cacheKey='shizuku-website-live-cache-v1';
  ...
  fetch(url, { cache:'no-cache', headers: { apikey: cfg.anonKey, Authorization: `Bearer ${cfg.anonKey}` } })
    ...
    .then(rows => { if(rows?.[0]?.data){try{localStorage.setItem(cacheKey,JSON.stringify({data:rows[0].data,cachedAt:Date.now()}))}catch(e){}window.SHIZUKU_APPLY_DATA(rows[0].data);} })
